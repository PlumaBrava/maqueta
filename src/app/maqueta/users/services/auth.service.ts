...
export class AuthService {
 ...
 //     var cleanEmail = auth.email.replace(/\./g, ',');
      ...
      this.getPefil(auth).subscribe(usuarioLogistica=>{
        ...
        this.setPerfilUsuarioObs(usuarioLogistica);
        // this.authState = auth;
...
// Returns current user display name or Guest
  get currentUserPhotoURL(): string {
     return this.authenticated ? this.authState['photoURL'] : false;

   }

    // Returns fecha de creacion del ususario
  get getFechaCreacion(): string {
    // console.log('creationTime',this.authState['metadata']['creationTime']);
    return this.authenticated ? this.authState['metadata']['creationTime'] : false;
  }
...
// Graba los datos del Ususario 


 public  setPerfilUsuarioObs(perfilUsuario:any ) {
...
private     getPerfilUsuarioSeleccionadoObs(): Observable<any> {
...
// cuando leo el perfil despues de tener el usuario logueado
 private   getPerfilUsuarioObs(): Observable<any> {
           return this.perfilUsuario.asObservable();
